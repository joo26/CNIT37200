...
Create a FUNCTION:
Calculate the LIKES-COMMENTS ratio of a specified video:
-Accepts videoID input within the procedures parameters
-Must output in the format ('Like-Comment Ratio: ')
...
CREATE OR REPLACE FUNCTION CalculateLikesCommentsRatio(video_id IN VARCHAR2)
RETURN VARCHAR2
IS
    likes          NUMBER;
    comments       NUMBER;
...
BEGIN
    IF video_id IS NULL OR NOT REGEXP_LIKE(video_id, '^[a-zA-Z0-9_]+$') THEN
    ...
    FROM VideoInfo
    WHERE VIDEOID = video_id;
    DBMS_OUTPUT.PUT_LINE('The Name of Video Chosen is: ' || video_title);
    IF comments = 0 THEN
        ratio := -1;
    ELSE
        ratio := likes / comments;
    END IF;
    IF likes > comments THEN
        RETURN 'There are more likes than comments. Like-Comment Ratio: ' || TO_CHAR(ratio);
    ELSIF likes < comments THEN
        RETURN 'There are more comments than likes. Like-Comment Ratio: ' || TO_CHAR(ratio);
    ELSE
        RETURN 'Likes and comments are equal. Like-Comment Ratio: ' || TO_CHAR(ratio);
    END IF;
...
DECLARE
    ...
    video_id        VARCHAR2(50) := 'b3x28s61q3c'; 
BEGIN
    result_message := CalculateLikesCommentsRatio(video_id);
...
The Name of Video Chosen is: The most EXPENSIVE thing I own.
There are more likes than comments. Like-Comment Ratio: 7677.9
...
It outputs the title of the video, and the statistics based on the parameters of the videoID.
...
DECLARE
    video_id      VARCHAR2(50) := 'wAZZ-UWGVHI'; 
...
BEGIN
    ...
    FROM EngagementMetrics
    WHERE VIDEOID = video_id;
    DBMS_OUTPUT.PUT_LINE('Video Name: ' || video_name);
    DBMS_OUTPUT.PUT_LINE('Total View Count: ' || total_views);
    DBMS_OUTPUT.PUT_LINE('Total Comment Count: ' || comment_count);
    DBMS_OUTPUT.PUT_LINE('Total Likes: ' || likes);
END;
/
...
Video Name: Apple Pay Is Killing the Physical Wallet After Only Eight Years | Tech News Briefing Podcast | WSJ
Total View Count: 135612
